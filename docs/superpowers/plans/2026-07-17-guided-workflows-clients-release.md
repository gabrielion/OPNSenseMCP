...
> The rewritten Guided plan must retain a mandatory native `windows-2025` pre-publication gate proving a
> package path containing spaces, `.cmd` launch, installer and ACL enforcement, MCP exchange, and
> mock connectivity, plus exact-process exit and cleanup evidence. Foundation and Provenance must not
