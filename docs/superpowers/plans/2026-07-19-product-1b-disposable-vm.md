...
- [ ] Add `scripts/vm/product1b.mjs doctor` and `npm run vm:doctor`.
- [ ] Check Node 22, macOS/Linux, `qemu-system-x86_64`, `qemu-img`, `curl`, and `bzip2`; print only fixed
  actionable capability results and make no download or filesystem mutation.
- [ ] Pin the official `OPNsense-26.1.6-nano-amd64.img.bz2` SHA-256
  `3c16267c791abfc3e41d5249fcb0c245c03cb91e2f1aa4d53017f0f3454d03a1` from the OPNsense 26.1 release
  page.
- [ ] Download only into a user cache, verify before decompression, keep the raw base immutable, and create a
...
- [ ] Start one QEMU process with serial console, loopback-only user networking/forwarding, bounded startup,
  an explicit PID/ownership record, and unconditional stop/residue proof.
- [ ] Bootstrap only the nano lab image through its serial console. Create a dedicated API key inside the
...
- [ ] Probe only `GET /api/core/system/status` and safe GET/POST variants of
...
- [ ] Select the single service-search request shape observed on OPNsense 26; update the versioned operation
  contract, generated descriptors, closed HTTPS client, mock, and tests together.
- [ ] Pack/install the same npm artifact and execute `opn_get system.status` plus
  `opn_list core.services` through MCP against the VM.
- [ ] Add `npm run test:product1b`, a sanitized machine-readable result, and concise README/CONTRIBUTING
