...
Point it at your own OPNsense in about 15 minutes, read-only. Needs Node.js 22.19 or newer within
major 22, on macOS or Linux:
...
Then ask _"what is my OPNsense system status?"_. Codex and OpenCode configurations are in the guide
below.

**[Connect your own OPNsense →](docs/setup-your-opnsense.md)** — the illustrated walkthrough: create a
least-privilege account, grant the three privileges the read tools need, issue the API key, trust the
self-signed certificate, wire Claude Code, Codex or OpenCode, and verify each step before the next.
Its screenshots are real captures of a pinned OPNsense, regenerated by `npm run docs:capture`.

Want to try it without touching a firewall? `npm run test:product1b` boots a disposable OPNsense VM,
proves the whole read surface against it and cleans up, with no credential of yours involved.
